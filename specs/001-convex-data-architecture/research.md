...
- **Decision**: node-media-server's play path embeds the publish stream key (`/live/<KEY>/index.m3u8`), so origin URLs are treated as secrets. Convex stores them but every non-admin read substitutes same-origin paths (`/stream/live/<streamId>.m3u8`, `/stream/vod/<streamId>.m3u8`). A Next.js route handler (`app/stream/[[...path]]/route.ts`) resolves streamId → origin URL via an internal Convex query and relays the playlist and its segments; the NMS host, port, and key exist only in server-side env/config.
