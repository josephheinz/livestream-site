...
## D15: Stream-key protection via same-origin HLS proxy (FR-022, SC-009)

- **Decision**: node-media-server's play path embeds the publish stream key (`/live/<KEY>/index.m3u8`), so origin URLs are treated as secrets. Convex stores them but every non-admin read substitutes same-origin paths: `/stream/live.m3u8` for the live stream (no ID — at most one is ever live, FR-002) and `/stream/vod/<streamId>.m3u8` for recordings. A Next.js route handler (`app/stream/[[...path]]/route.ts`) resolves the path → origin URL via an internal Convex query and relays the playlist and its segments; the NMS host, port, and key exist only in server-side env/config.
- **Rationale**: A leaked play URL would otherwise be a working *broadcast* credential — viewers could hijack the stream. Same-origin proxying closes that and also enforces VOD privacy at the file level for proxied paths (404 for private VODs).
- **Alternatives considered**: Exposing NMS directly with publish-auth (sign secret) enabled — still leaks origin host and invites direct scraping; recommended as *complementary* ops hardening (spec assumption), not the fix. Renaming stream keys per broadcast — rotation hygiene, not protection.
- **Note**: Proxying live HLS through a Next.js function adds per-viewer bandwidth through Vercel. Fine at this site's scale; if it ever hurts, swap the route handler for a dumb reverse proxy (nginx/Caddy on the media host) serving the same `/stream/*` contract — nothing else changes.
